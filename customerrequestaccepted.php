...
$result=mysqli_query($con,"select concat(user.first_name,' ',user.last_name),user.mob_no from user inner join auto on user.user_id=auto.driver_id INNER join request on request.auto_id=auto.auto_id where request.request_id=".$requestid);
$row=mysqli_fetch_row($result);
$drivername=$row[0];
$drivermobilenoo=$row[1];
?>
<!DOCTYPE html>
<html lang="en">
...
function load(){
            var xmlhttp = new XMLHttpRequest();
                    ...
                    window.location.href="tripcost.php?requestid=<?=$requestid?>";
                    else 
                    setTimeout(load,700);
                    }
                    };
        ...
        <center>
          <h2>Request Accepted</h2>
                ...
                <center> <div class="form-group" >
                    <h1> Your request is Accepted by<br><?=$drivername?><br>
                    <?=$drivermobilenoo?>
                </h1>
