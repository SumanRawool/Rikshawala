...
$result=mysqli_query($con,"select concat(user.first_name,' ',user.last_name),request.source,request.destination from user inner join request on user.user_id=request.customer_id where request.request_id=".$_GET["requestid"]);
...
if(isset($_POST["submit"]))
{
    ...
    $cost=$_POST["cost"];
mysqli_query($con,"insert into trip(trip_cost,request_id,trip_date) values($cost,$requestid,sysdate())");
mysqli_query($con,"update request set request_status='Completed' where request_id=".$requestid);
...
<form action="activetrip.php" method="POST">
    <div class="container">
      <div id="mdb-preloader" class="flex-center">
        ...
        <h6> From:<?=$row[1]?></h6></span>
        <span style="float:right"><h6 style="padding-left: 129px;">To:<?=$row[2]?></h6></span>
        </div>     
            <label for="cost">Cost </label>
                <input type="number" id="cost" class="form-control" name="cost" placeholder="Cost" required><br>
                          
                <center> <div class="form-group" >
            <input type="hidden" name="requestid" value="<?=$requestid?>">
              <input type="submit" style="background-color: #f7c60b;align:center" id="submit" class="btn btn-lg" name="submit" value="Complete Trip"><br>
